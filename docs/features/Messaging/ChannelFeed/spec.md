...
- [x] **E2E:** Playwright opens a channel page (seeded with messages), asserts the message list renders; scrolls to the top of the list, asserts older messages are prepended (infinite scroll trigger fires).
  - **E2E automation deferred (feat-001 scope):** The `ChannelFeed` React component and TanStack Query `useInfiniteQuery` hook are built in feat-005/feat-006; the Playwright automation lives in feat-009 (`e2e-channel-feed-playwright`) which runs after all UI features are complete.
  - **UAT (manual gate for feat-001):** Start `reddit-clone-api` and send `GET /channels/{channelId}/messages` with a valid Bearer JWT (member of the channel). Verify: 200 response with `{ messages: [...], nextCursor: string | null }` shape, messages in descending `sequenceNumber` order, `nextCursor` non-null when >50 messages exist.
